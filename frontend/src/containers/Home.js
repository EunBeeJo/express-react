...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    const loadMemoLoop = () => {
      this.loadNewMemo().then(
        () => {
          this.memoLoaderImeoutId = setTimeout(loadMemoLoop, 5000);
        }
      );
    };
  ...
  componentWillUnMount() {
    ...
    clearTimeout(this.memoLoaderImeoutId);
      ...
      () => {
        if (this.props.editStatus.status === "SUCCESS") {
          ...
          */
          ...
          // NOTIFY ERROR
          let $toastContent = $('<span style="color: #FFB4BA">' + errorMessage[error - 1] + 'error num : ' + error + '</span>');
          Materialize.toast($toastContent, 2000);
  ...
  handleRemove(id, index) {
    this.props.memoRemoveRequest(id, index).then(() => {
      if (this.props.removeStatus.status === "SUCCESS") {
        ...
        // 1 SECON LATER. (ANIMATION TAKES 1SEC)
        setTimeout(() => {
          if ($("body").height() < $(window).height()) {
            this.loadOldMemo();
          }
